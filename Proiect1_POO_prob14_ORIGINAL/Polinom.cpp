...
Polinom::Polinom(int grad, bool fisier) {
    this->grad = grad;
    setHead();
    setFeet();
    this->setNr_Polinoame(this->getNr_Polinoame()+1);
    int aux = grad;
    if(fisier){
        cout<<"Gradul polinomului este: "<<aux;
        cout<<endl<<"citim din consola..."<<endl;
        while(aux!=-1){
            Nod *a = new Nod();
            int coef; f>>coef;
            a->setCoef(coef); a->setExponent(aux);
...
const Polinom Polinom::operator--(int dummy) {
    Nod *a = this->getHead();
    while(a != nullptr){
        a->setCoef(a->getCoef()-1);
        a=a->getNext();
    }
    return *this;
}
...
Polinom Polinom::operator--() {
    Nod *a = this->getHead();
    while(a != nullptr){
        //trebuia sa suprascriu si operatorii la nod ca sa mearga si asta
        a->setCoef(a->getCoef()-1);
        a=a->getNext();
    }
    return *this;
...
int  Polinom::operator!=(const Polinom &other) {
    Nod *a = this->getHead();
    Nod *b = other.getHead();
...
/*
float Polinom::operator float() {
    Nod *a = this->getHead();
    while(a != nullptr){
        a->setCoef(float(a->getCoef()));
        a=a->getNext();
    }
    return *this;
}
...
int Polinom::operator[](int i) {
    ...
    cout<<"Nu s-a gasit elementul de pe pozitia cautata"<<endl;
    return -1;
}
